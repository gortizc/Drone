...
#define CKGR_MOR_Val    0x01370809      // Main Cristal Oscillator is selected
#define CKGR_PLLR_Val   0x20200003      // 0x00003F00
#define PMC_MCKR_Val    0x00000012      // PLLA Clock is selected
...
static void SetDefaultMaster(unsigned char enable)
{
   ...
   if (enable == 1) 
   {
      ...
      MATRIX->HMATRIX2_SCFG0 |= AT91C_MATRIX_FIXED_DEFMSTR_SCFG_ARMS |
                                AT91C_MATRIX_DEFMSTR_TYPE_FIXED_DEFMSTR;
        
      // Set default master: SRAM1 -> Cortex-M3 System
      MATRIX->HMATRIX2_SCFG1 |= AT91C_MATRIX_FIXED_DEFMSTR_SCFG_ARMS |
                                AT91C_MATRIX_DEFMSTR_TYPE_FIXED_DEFMSTR;

      // Set default master: Internal flash0 -> Cortex-M3 Instruction/Data
      MATRIX->HMATRIX2_SCFG3 |= AT91C_MATRIX_FIXED_DEFMSTR_SCFG_ARMC |
                                AT91C_MATRIX_DEFMSTR_TYPE_FIXED_DEFMSTR;
    } 
    else 
    {
      // Clear default master: SRAM0 -> Cortex-M3 System
      MATRIX->HMATRIX2_SCFG0 &= (~AT91C_MATRIX_DEFMSTR_TYPE);
        
      // Clear default master: SRAM1 -> Cortex-M3 System
      MATRIX->HMATRIX2_SCFG1 &= (~AT91C_MATRIX_DEFMSTR_TYPE);

      // Clear default master: Internal flash0 -> Cortex-M3 Instruction/Data
      MATRIX->HMATRIX2_SCFG3 &= (~AT91C_MATRIX_DEFMSTR_TYPE);
   }
}
#endif /* defined(SAM3U_PLATFORM) */
...
void SystemCoreClockUpdate (void)            /* Get Core Clock Frequency      */
{
  ...
  switch (PMC->PMC_MCKR & 3) {
    ...
    case 2:                             /* PLL  clock                         */
      ...
      SystemCoreClock *= ((((PMC->CKGR_PLLR) >> 16) & 0x7FF) + 1);
      SystemCoreClock /= ((((PMC->CKGR_PLLR) >>  0) & 0x0FF));
      break;
    case 3:                             /* Reserved                           */
      SystemCoreClock  = 0;
      break;
...
void SystemInit (void)
{
  ...
  PMC->CKGR_PLLR = CKGR_PLLR_Val | (1 << 29);
